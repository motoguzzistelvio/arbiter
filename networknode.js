...
app.post('/transaction', function(req, res) {
  const blockIndex = bitcoin.addTransactionToPendingTransactions(createTransaction(req));
  buff = null;
...
app.post('/transaction/broadcast', function(req, res)  {
  var newTransaction = createTransaction(req);
  var idx = bitcoin.addTransactionToPendingTransactions (newTransaction);
  ...
  Promise.all(requestPromises)
  .then(data => {
    res.json({ note: 'Transaction created and broadcast successfully. added to block '});
  });
});
...
function createTransaction(req){
  var fileHash  = null;
  if (buff != null){
    fileHash = bitcoin.hashFile(buff);
  }
  return bitcoin.createNewTransaction(req.body.amount,
                                      req.body.fileName,
                                      buff==null?req.body.fileHash:fileHash, 
                                      req.body.sender, 
                                      req.body.recipient);
}
